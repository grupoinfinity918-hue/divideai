...
export default function Home(){
 const [settings,setSettings]=useState({aiChatEnabled:true,homepageOrder:['OWN','THIRD_PARTY'],activeTheme:'DEFAULT'}); const[own,setOwn]=useState([]),[third,setThird]=useState([]),[cats,setCats]=useState([]),[cat,setCat]=useState(null);
 ...
 return <div className="min-h-screen bg-white"><Header/><main className="max-w-7xl mx-auto px-4 pb-20"><div className="market-search"><span>⌕</span><input placeholder="Busque streaming, serviços, ofertas..."/><kbd>⌘ K</kbd></div><div className="category-strip"><button onClick={()=>setCat(null)} className={!cat?'cat-active':'cat'}>Tudo</button>{cats.map(c=><button key={c.id} onClick={()=>setCat(c.slug)} className={cat===c.slug?'cat-active':'cat'}>{c.name}</button>)}<Link to="/ranking" className="cat">🏆 Ranking</Link></div><BannerCarousel theme={settings.activeTheme}/>{settings.homepageOrder.map(k=><Rail key={k} {...sections[k]} />)}<section className="grid md:grid-cols-3 gap-4 py-8"><div className="feature-card"><b>Pagamento Pix</b><span>Checkout rápido e confirmação automática.</span></div><div className="feature-card"><b>Compra acompanhada</b><span>Tenha protocolo e histórico do seu pedido.</span></div><div className="feature-card"><b>Vendedores avaliados</b><span>Escolha serviços com reputação e avaliações.</span></div></section></main><FloatingAIChat enabled={settings.aiChatEnabled}/><InstallPrompt/></div>;
}
