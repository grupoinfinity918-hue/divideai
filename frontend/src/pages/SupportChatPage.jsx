...
export default function SupportChatPage() {
  ...
  async function load() {
    const res = await fetch(`/api/support-chats/${id}`, { headers: authHeader() });
    if (res.ok) setChat(await res.json());
  }

  async function send() {
    if (!input.trim()) return;
    await fetch(`/api/support-chats/${id}/messages`, {
      method: 'POST',
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-lg mx-auto px-4 pt-8 pb-16">
        <div className="card p-5 h-[520px] flex flex-col">
          <h1 className="font-bold text-gray-900 mb-1">Suporte Divide Aí</h1>
          <p className="text-xs text-gray-400 mb-3">Protocolo: {chat.protocol}</p>

          <div className="flex-1 overflow-y-auto flex flex-col gap-2">
            {chat.messages?.length === 0 && (
              <p className="text-sm text-gray-400">
                Aguarde, um atendente vai te responder em breve. Enquanto isso, pode deixar sua mensagem.
              </p>
            )}
            {chat.messages?.map(m => (
              <div
